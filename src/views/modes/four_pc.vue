  ...
  <div id="genericbg" class="text-center text-white">
    <b-container class="vertical-center">
      <b-row>
        ...
        >
        <b-col
          ><h1 class="big-text">Blue</h1>
          <b-button
            pill
            class="btn-big"
            :pressed.sync="ch02.mute"
            data-mode="ONE"
            v-on:click="mutech02()"
            variant="danger"
            >{{ ch02.text }}</b-button
        ...
        >
        <b-col
          ><h1 class="big-text">Red</h1>
          <b-button
            pill
            class="btn-big"
            :pressed.sync="ch03.mute"
            data-mode="ONE"
            v-on:click="mutech03()"
            variant="danger"
            >{{ ch03.text }}</b-button
        ...
        >
        <b-col
          ><h1 class="big-text">Green</h1>
          <b-button
            pill
            class="btn-big"
            :pressed.sync="ch04.mute"
            data-mode="ONE"
            v-on:click="mutech04()"
            variant="danger"
            >{{ ch04.text }}</b-button
...
export default {
  ...
  methods: {
    ...
    doMuteHeartbeat: function () {
      ...
      axios({
        method: "get",
        url: `${vm.$parent.API_BASE_URL}/x32/ch/02/mute`,
      })
        .then(function (response) {
          if (response.data.error) {
            console.error(response.data.message);
      ...
      axios({
        method: "get",
        url: `${vm.$parent.API_BASE_URL}/x32/ch/03/mute`,
      })
        .then(function (response) {
          if (response.data.error) {
            console.error(response.data.message);
      ...
      axios({
        method: "get",
        url: `${vm.$parent.API_BASE_URL}/x32/ch/04/mute`,
      })
        .then(function (response) {
          if (response.data.error) {
            console.error(response.data.message);
